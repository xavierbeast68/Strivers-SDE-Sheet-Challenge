...
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        ...
        // return obj.brute1(matrix, target);

        //* Brute Force-> making use of row sorted property
        // binary search on every row
        // TC-> O(rows*log(cols))
        // return obj.brute2(matrix, target);

        //* Better-> if row-wise sorted and column-wise sorted
        // staircase search
        //* TC-> O(n + m)
        // return obj.better(matrix, target);

        //* Optimized Approach-> if matrix is combined row-wise, becomes a sorted array, perform binary search on that
        // TC = O(log(n*m)), SC= O(1)
        return obj.optimized(matrix, target);
    }
